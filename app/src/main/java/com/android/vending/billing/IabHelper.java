...
public class IabHelper {
    ...
    public void startSetup(final OnIabSetupFinishedListener listener) {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                try {
                    ...
                    if (mSubscriptionsSupported) {
                        ...
                        if (response == BILLING_RESPONSE_RESULT_OK) {
                            logDebug("Subscription v5 AVAILABLE.");
                            mSubscriptionUpdateSupported = true;
                        } else {
                            logDebug("Subscription v5 NOT AVAILABLE.");
                            mSubscriptionUpdateSupported = false;
                        }
                    }
        ...
        if (intentServices != null && !intentServices.isEmpty()) {
            ...
            if (listener != null) listener.onIabSetupFinished(new IabResult(BILLING_RESPONSE_RESULT_BILLING_UNAVAILABLE, "Billing service unavailable on device."));
        }
    }

    public void dispose() throws IabAsyncInProgressException {
        synchronized (mAsyncInProgressLock) {
            if (mAsyncInProgress) throw new IabAsyncInProgressException("Can't dispose because an async operation (" + mAsyncOperation + ") is in progress.");
        }
        logDebug("Disposing.");
        mSetupDone = false;
    ...
    public void launchPurchaseFlow(Activity act, String sku, String itemType, List<String> oldSkus, int requestCode, OnIabPurchaseFinishedListener listener, String extraData) throws IabAsyncInProgressException {
        ...
        try {
            ...
            e.printStackTrace();
            flagEndAsync();
            IabResult result = new IabResult(IABHELPER_SEND_INTENT_FAILED, "Failed to send intent.");
            if (listener != null) listener.onIabPurchaseFinished(result, null);
        } catch (RemoteException e) {
    ...
    void flagStartAsync(String operation) throws IabAsyncInProgressException {
        synchronized (mAsyncInProgressLock) {
            if (mAsyncInProgress) throw new IabAsyncInProgressException("Can't start async operation (" + operation + ") because another async operation (" + mAsyncOperation + ") is in progress.");
    ...
    int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus) throws RemoteException, JSONException {
        ...
        if (moreSkus != null) {
            for (String sku : moreSkus) {
                if (!skuList.contains(sku)) skuList.add(sku);
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (String s : skuList.subList(i * 20, i * 20 + 20)) tempList.add(s);
            packs.add(tempList);
        }
        if (mod != 0) {
            ArrayList<String> tempList = new ArrayList<>();
            for (String s : skuList.subList(n * 20, n * 20 + mod)) tempList.add(s);
            packs.add(tempList);
        }
